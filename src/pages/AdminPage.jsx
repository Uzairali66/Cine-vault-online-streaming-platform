  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const [trendingRes, movieCountRes, tvCountRes] = await Promise.all([
          fetch(`${BASE}/trending/all/day?language=en-US&page=1`, opts),
          fetch(`${BASE}/discover/movie?language=en-US&page=1&sort_by=popularity.desc`, opts),
          fetch(`${BASE}/discover/tv?language=en-US&page=1&sort_by=popularity.desc`, opts),
        ]);

        const trending = await trendingRes.json();
        const movies = await movieCountRes.json();
        const tv = await tvCountRes.json();
                ...
                <div className="bg-white/5 rounded-xl p-4">
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        type="password"
                        value={import.meta.env.VITE_TMDB_API_KEY ? '••••••••••••••••' : 'Not configured'}
                        readOnly
                      ...
                      />
                      <p className="text-gray-500 text-xs mt-1">Configured via <code className="text-blue-300 bg-blue-900/30 px-1 rounded">VITE_TMDB_API_KEY</code> in .env.local</p>
                    </div>
                    <div>
